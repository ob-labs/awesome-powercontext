...
PowerContext evidence: ADD operation with `memory_kind=cabin_control_preference`.
Vehicle evidence: no state diff is required in this act.
Privacy evidence: raw utterance is not stored as long-term memory.
Talk track: PowerContext stores derived memory, not a raw transcript.
...
PowerContext evidence: SEARCH filters include actor and seat position.
...
PowerContext evidence: SEARCH returns `cabin_control_preference` and `driving_preference` memories.
...
Talk track: PowerContext turns repeated actions into procedural memory.
...
PowerContext evidence: SEARCH returns `vehicle_capability` with unsupported feature metadata.
...
PowerContext evidence: SEARCH returns `location_episode` with generalized POI content.
...
PowerContext evidence: SEARCH returns `media_preference` and `safety_policy`.
...
Talk track: PowerContext combines passenger context and media history.
...
PowerContext evidence: SEARCH returns `relationship_event` and safe place preferences.
...
PowerContext evidence: SEARCH returns `driving_preference` and `emotional_preference`.
...
PowerContext evidence: SEARCH returns charging reminder tone preference.
...
PowerContext evidence: `temporary_context` moves to `decayed`; long-term preference stays active.

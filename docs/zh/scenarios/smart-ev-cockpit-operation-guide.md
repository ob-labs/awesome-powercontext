...
2. 如需使用自定义 PowerMem 用户身份，先在座位身份面板设置驾驶员 / 乘客 / 后排儿童的 `user_id`。后续生成和导入都会把 actor 映射到当前最新的 `user_id`。
...
5. 点击 **导入**（写入 PowerMem，并发 8 线程）。状态显示"已导入 X / N 条"，导入过程涉及 LLM 抽取和向量化，**需要几分钟，请在演示开始前预留时间**。
6. 状态变为"已导入"后即可开始演示。
...
| 幕 | 天数 | 说话人 | 台词（自动预填） | 看点 |
|---|---|---|---|---|
| 场景 1 建立记忆 | 第 1 天 | 主驾 | 按当前季节生成；夏季示例：我夏天上车一般 23C，座椅加热 0 档。 | 证据面板出现 **ADD** 操作，`memory_kind=cabin_control_preference`；保存的是抽取后的偏好，不是聊天原文 |
| 场景 2 同一句话，不同人 | 第 7 天 | 主驾（**手动再切前排乘客、后排儿童各发一次**） | 按当前季节生成；夏季示例：车里有点热。 | 三人得到不同车控 patch；儿童命中安全策略记忆；SEARCH filters 里带 actor + 座位 |
| 场景 3 组合车控例程 | 第 14 天 | 主驾 | 按我上次舒服的设置来。 | 检索到多条偏好记忆组合执行；车辆状态面板出现 HVAC/座椅加热 before/after diff |
...
收尾动作：点 **导出** 下载完整 trace JSON，向观众展示全程每一次 PowerMem 操作都可追溯。

**Presenter 铁律**（来自演示手册）：只讲后端真实返回的 memory hits 和 trace evidence；如果 PowerMem 不可用，页面会显示 live-mode 错误——此时停止演示排查问题，不要脱稿编造。
...
| 页面顶部出现 live-mode 错误条 | 后端挂了或 PowerMem 初始化失败。看 `logs/backend.log`，常见原因：存储后端连不上（OceanBase 模式检查数据库端口）、LLM 服务超时 |
| 场景 5–9 检索不到记忆（memory hits 为空） | 测试数据没导入。按第 3 节生成 + 导入 |
| 发送后长时间转圈 | LLM 服务慢或不通，用第 2 节第③项验证；后端日志会有超时报错 |
| 导入进度卡住 | 看 `test-data/status` 里的 `failed_count` 和 `last_error`；LLM 限流时降低 `max_workers` 重试 |
...
（如果记忆存储用的是共享 OceanBase 集群，只清数据集即可，**不要**停数据库进程。）
